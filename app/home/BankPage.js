...
export default function BankPage({ user }) {
  ...
  const [recentInterest, setRecentInterest] = useState(0); // animation
  const interestRef = useRef(0);

  const RATE_PER_TICK = 0.00002; // 0.002% per 2s
  const TICK_DURATION = 5;

  const fmt = (num) =>
    new Intl.NumberFormat("en-US", { maximumFractionDigits: 0 }).format(num || 0);

  // Load initial data + calculate offline interest
  useEffect(() => {
    if (!user) return;
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      // Save accrued interest & update last_interest_date
      await supabase
        .from("bank")
        .update({
  ...
  const handleDeposit = async () => {
    ...
    if (!amt || amt < 50000 || amt > coin) return alert("Invalid amount. Minimum deposit is 50,000");
  ...
  const handleWithdraw = async () => {
    if (!bank || bank.balance <= 0) return alert("No funds to withdraw");
    ...
    await supabase.from("bank_transactions").insert([
      ...
      },
      {
        user_id: user.id,
        type: "interest",
        amount: bank.accrued_interest || 0,
        date: new Date(),
      },
    ]);
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-purple-100 to-blue-100 rounded-xl shadow-lg">
      <h2 className="text-3xl font-bold mb-4 flex items-center gap-3">
        <PiggyBank className="text-purple-600" /> Bank
      </h2>

      <div className="text-sm mb-3 flex items-center gap-2 text-gray-600">
        <Percent size={16} /> Interest rate: 0.002% / 2s
      </div>

      <div className="grid grid-cols-2 gap-6 mb-6">
        <div className="bg-white p-6 rounded-xl shadow text-center">
          <p className="text-gray-500">Current Coins</p>
          <p className="text-2xl font-bold text-blue-600">{fmt(coin)}</p>
        </div>
        <div className="bg-white p-6 rounded-xl shadow text-center relative">
          <p className="text-gray-500">Deposit</p>
          <p className="text-2xl font-bold text-purple-600">{fmt(bank?.balance)}</p>
        </div>
        <div className="bg-white p-6 rounded-xl shadow text-center relative">
          <p className="text-gray-500">Accumulated Interest</p>
          <p className="text-2xl font-bold text-green-600">{fmt(interestPreview)}</p>
          {recentInterest > 0 && (
            <span className="absolute top-2 right-2 text-green-700 font-bold animate-bounce">
              +{fmt(recentInterest)}
            </span>
          )}
        </div>
        <div className="bg-white p-6 rounded-xl shadow text-center">
          ...
          <p className="text-2xl font-bold text-emerald-600">{fmt((bank?.total_interest || 0) + interestPreview)}</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        <input
          ...
          className="border px-3 py-2 rounded w-48 focus:outline-blue-400"
        />
        <button
          onClick={handleDeposit}
          className="flex items-center gap-2 bg-blue-500 text-white px-6 py-3 rounded-xl hover:bg-blue-600 transition"
        >
      ...
      <h3 className="font-semibold mb-3 text-lg">📜 Transaction History</h3>
      <ul className="space-y-1 max-h-72 overflow-y-auto bg-white p-4 rounded-xl shadow">
        {transactions.length === 0 ? (
          <li className="text-gray-500 text-center py-2">No transactions yet</li>
        ) : (
          transactions.map((tx) => (
            <li key={tx.id} className="flex justify-between border-b py-2 text-sm items-center">
              <span
              ...
              >
                {tx.type === "deposit" ? "💰 Deposit" : tx.type === "withdraw" ? "🏧 Withdraw" : "📈 Interest"}
              </span>
              <span className="font-semibold">{fmt(tx.amount)}</span>
              <span className="text-gray-500 text-xs">{dayjs(tx.date).format("HH:mm:ss DD/MM/YYYY")}</span>
            </li>
          ))
        )}
      </ul>
