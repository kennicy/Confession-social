    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (userPostIds.map(String).includes(String(newNotif.reference_id))) {
    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      <div className="flex gap-2 mb-4 items-center flex-wrap">
        ...
        <button
          ...
          className="ml-auto px-3 py-1 rounded-full bg-green-500 text-white hover:bg-green-600"
        >
          Mark all as read
        </button>
        <button
          onClick={clearRead}
          className="px-3 py-1 rounded-full bg-red-500 text-white hover:bg-red-600 flex items-center gap-1"
        >
          ...
          {filtered.map((notif) => (
            <motion.div
              ...
              className={`relative flex items-start p-4 rounded-xl shadow-sm hover:shadow-md transition-all ${
                notif.is_read ? "bg-gray-50" : "bg-white border-l-4 border-blue-500"
              }`}
            >
              {/* NEW badge */}
              {!notif.is_read && (
              ...
              <div className="mr-4 mt-1">
                {notif.type === "reaction" && <Heart className="w-6 h-6 text-red-500" />}
                {notif.type === "reply" && <MessageCircle className="w-6 h-6 text-green-500" />}
                {notif.type === "cfs" && <Zap className="w-6 h-6 text-yellow-500" />}
              </div>
              <div className="flex-1">
                <p className="text-sm text-gray-500">{timeAgo(notif.created_at)}</p>
                <p className="mt-1 font-medium">{getNotificationText(notif)}</p>
              </div>
              ...
              <div className="ml-3 flex items-center gap-2">
                ...
                {notif.is_read && <CheckCircle className="w-5 h-5 text-gray-400" />}
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
