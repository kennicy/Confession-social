...
export default function CalendarComponent() {
  ...
  return (
    <div style={{ padding:20, fontFamily:"Inter, Arial, sans-serif" }}>
      <h2 style={{ marginBottom:15, fontWeight:600, fontSize:20 }}>📅 Timetable Calendar</h2>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:20 }}>
        <textarea
          ...
          style={{ flex:1, fontFamily:"monospace", fontSize:12, padding:10, borderRadius:6, border:"1px solid #ccc", resize:"vertical" }}
        />
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          <button onClick={parseText} style={{ padding:"10px 16px", borderRadius:6, background:"linear-gradient(90deg,#4A90E2,#357ABD)", color:"#fff", border:"none", cursor:"pointer" }}>Generate Calendar</button>
          <button onClick={saveCalendar} style={{ padding:"10px 16px", borderRadius:6, background:"linear-gradient(90deg,#28a745,#218838)", color:"#fff", border:"none", cursor:"pointer" }}>Save Calendar</button>
        </div>
      </div>
      ...
      {/* Calendar */}
      <div style={{ borderRadius:10, overflow:"hidden", boxShadow:"0 6px 20px rgba(0,0,0,0.15)" }}>
        <FullCalendar
          ref={calendarRef}
          ...
          eventDidMount={info=>{
            info.el.setAttribute("title",
              `${info.event.title}\nGroup ${info.event.extendedProps.nhom}\nRoom: ${info.event.extendedProps.phong}\nCampus: ${info.event.extendedProps.cs}\nWeek: ${info.event.extendedProps.week}\nPeriod: ${info.event.extendedProps.tiet}\nTime: ${info.event.extendedProps.gio}`
            );
          }}
        />
      </div>
      ...
      <div
        style={{
          ...
          pointerEvents: "none" // tránh che button phía sau
        }}
      >
        {popupMsg}
      </div>
