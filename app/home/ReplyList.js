...
export default function ReplyList({ confession, user, profile, formatTimeAgo }) {
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "flex-start", position: "relative" }}
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "4px" }}>
            <div style={{
              ...
              <b style={{ fontSize: "0.85em" }}>{r.is_anonymous ? "Ẩn danh" : r.profiles?.full_name}</b>
              <div>{r.content}</div>
            </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      ...
      <div ref={el => replyRefs.current[confession.id] = el} style={{ display: "flex", alignItems: "center", gap: "10px", marginTop: "6px", background: "#f1f2f6", padding: "8px 12px", borderRadius: "30px", position: "relative" }}>
        <div style={{ width: 36, height: 36, borderRadius: "50%", overflow: "hidden", flexShrink: 0 }}>
        ...
        <input type="text" placeholder="Trả lời..." value={replyInputs[confession.id] || ""}
          ...
          style={{ flex: 1, borderRadius: "22px", border: "1px solid #ccc", padding: "8px 12px", fontSize: "0.9em" }}
        />
        <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
          ...
          {showEmoji[confession.id] && (
            <div style={{ position: "absolute", bottom: "45px", left: "-250px", zIndex: 20, boxShadow: "0 4px 12px rgba(0,0,0,0.15)", borderRadius: "12px", background: "#fff" }}>
        ...
        <button onClick={() => addReply(confession.id, replyAnonymous[confession.id] || false)} disabled={isPosting[confession.id]} style={{ padding: "5px 12px", borderRadius: "14px", background: "#4caf50", color: "#fff", fontSize: "0.85em", cursor: isPosting[confession.id] ? "not-allowed" : "pointer" }}>{isPosting[confession.id] ? "Đang gửi..." : "Rep"}</button>
      </div>
      ...
      {deleteConfirm && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center" }}>
          <div style={{ background: "#fff", padding: "28px", borderRadius: "14px", textAlign: "center" }}>
