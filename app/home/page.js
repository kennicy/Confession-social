...
export default function HomePage() {
  ...
  const [selectedUserId, setSelectedUserId] = useState(null); // 👈 thêm
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#f9fbfd" }}>
      <LeftSidebar tab={tab} setTab={(newTab) => {
          setTab(newTab);
          setSelectedUserId(null); // reset nếu click tab trái
        }} user={user} setUser={setUser} />
      <MainContent tab={tab} setTab={setTab} user={user} profile={profile} setProfile={setProfile} selectedUserId={selectedUserId}/>
      <RightSidebar
        user={user}
        profile={profile}
        onSelectUser={(id) => setSelectedUserId(id)} // ← thêm dòng này
      />
